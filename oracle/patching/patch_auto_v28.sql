...
for ORATAB_OH in $UNIQUE_HOMES; do
    ...
    # --------------------------------------------------------
    OPATCH_ZIP=$(ls -1 "$SETUP_BASE"/p6880880_*.zip 2>/dev/null | head -1)
    ...
    if [[ -n "$MANUAL_COMBO_PATCH_NUM" ]]; then
        COMBO_ZIP=$(ls -1 p${MANUAL_COMBO_PATCH_NUM}_*.zip 2>/dev/null | grep -v "p6880880" | head -1)
    else
        COMBO_ZIP=$(ls -1 p*.zip 2>/dev/null | grep -v "p6880880" | head -1)
    fi
    ...
    PATCH_NUM=$(echo "$COMBO_ZIP" | cut -d'_' -f1 | tr -d 'p')
    ...
    # DBRU Patch dizinini bul (-mindepth 1 eklenerek ana klasörün seçilmesi engellendi)
    DBRU_DIR=$(find "$SETUP_BASE/$PATCH_NUM" -mindepth 1 -maxdepth 1 -type d | grep -v OJVM | grep -E "[0-9]{8}$" | head -1)
    ...
    if [[ -n "$DBRU_DIR" ]]; then
        ...
        log "WARNING: Could not identify DBRU patch directory inside $PATCH_NUM"
    fi

    # OJVM Patch dizinini bul
    OJVM_DIR=""
    if [[ -n "$MANUAL_OJVM_PATCH_NUM" ]]; then
        OJVM_DIR=$(find "$SETUP_BASE/$PATCH_NUM" -mindepth 1 -maxdepth 1 -type d -name "$MANUAL_OJVM_PATCH_NUM" | head -1)
    else
        OJVM_DIR=$(find "$SETUP_BASE/$PATCH_NUM" -mindepth 1 -maxdepth 1 -type d | grep -v "$DBRU_DIR" | grep -E "[0-9]{8}$" | head -1)
    fi
